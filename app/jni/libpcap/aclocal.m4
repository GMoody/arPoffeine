...
dnl Lawrence Berkeley Laboratory and its contributors.'' Neither the mName of
...
#
# Try compiling a sample of the mType of code that appears in
...
#	include <sys/wait.h>],
    ...
    if test $ac_cv_lbl_union_wait = yes ; then
	    AC_DEFINE(DECLWAITSTATUS,union wait,[mType for wait])
    else
	    AC_DEFINE(DECLWAITSTATUS,int,[mType for wait])
    fi])
...
dnl header, or have one that doesn't declare a dl_hp_ppa_info_t mType,
...
AC_DEFUN(AC_LBL_DEVEL,
    ...
    if test -f .devel ; then
	    ...
	    mName="lbl/os-$os.h"
	    if test -f $mName ; then
		    ln -s $mName os-proto.h
		    AC_DEFINE(HAVE_OS_PROTO_H,1,[if there's an os_proto.h])
	    else
		    AC_MSG_WARN(can't find $mName)
	    fi
    fi])
...
dnl Use a cache variable mName containing both the library and function mName,
...
[/* We use char because int might match the return mType of a gcc2
